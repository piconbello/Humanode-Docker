...
#   - runtime start against an empty volume exits with a clear error
#
...
echo "check: runtime start on empty volume exits non-zero with clear message"
VOL2="hmnd-test-empty-$$"
docker volume create "$VOL2" > /dev/null
OUT="$(docker run --rm -v "$VOL2:/data" "$IMAGE" 2>&1 || true)"
docker volume rm "$VOL2" > /dev/null 2>&1 || true
echo "$OUT" | grep -q "keystore not found" \
    || fail "runtime start should surface 'keystore not found' error; got: $OUT"
